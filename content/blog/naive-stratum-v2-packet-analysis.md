...
* Stratum V2 encrypts the protocol contents, but **encryption does not make the traffic completely invisible**. A passive observer can combine packet sizes, timing, direction, protocol state, and implementation details to make increasingly confident guesses about what is happening inside the connection.
...
* I also learned about the [**PUSH bit**](https://datatracker.ietf.org/doc/html/rfc9293#name-send) in TCP.
...
For this experiment, I ran both the pool and the miner on the same machine. I did this to keep the capture clean and avoid unrelated traffic in Wireshark.

I used Wireshark to capture the traffic between the pool and the miner:
...
This is not a typical production deployment. In a real setup, the pool and miner would usually be separated by a network, with proxies, switches, routers, and unrelated traffic between them.

This is why I call this a **naive analysis**. I started with the simplest possible environment to see what the packet patterns alone reveal.
...
The first four packets were the TCP three-way handshake plus a TCP Window Update. This is TCP behavior, not Stratum V2 behavior, as the Wireshark output shows:
...
This is an improvement over Stratum V1 packets, whose contents were unencrypted JSON. For example, this packet exposes its contents:
...
After a quick look at [TCP RFC 9293](https://datatracker.ietf.org/doc/html/rfc9293), I found that `PSH` is one of the TCP control bits:
...
#### Things we can infer from the packets, with some cheating

When I started this experiment, I expected packet lengths to reveal some message flows. A passive observer who does not know which traffic to inspect may not get much from that alone. So I decided to cheat a little.

I matched the packet lengths with message lengths from the publicly available Stratum V2 specification, using commit [0f38d51](https://github.com/stratum-mining/sv2-spec).

This lets us infer when the first two acts of the Noise protocol occurred.
...
From this, I can make the following guess:
...
At the time of writing, the Noise handshake in the Stratum V2 specification contains no 234-byte message. That would break this heuristic.

However, section [4.5.2.1](https://github.com/stratum-mining/sv2-spec/blob/0f38d51dcd569e8f76575bf03cdf12848f4a4bef/04-protocol-security.md?plain=1#l247-l273) says that act 2 has 170 bytes. The capture shows 234 bytes. I opened [issue #215](https://github.com/stratum-mining/sv2-spec/issues/215) to report the discrepancy. For now, I am treating the capture as the correct value.

An observer who matches this traffic against the specification can make a better guess about what is happening in the connection. Packet sizes and the time between them can provide reasonable confidence that the packets correspond to specific parts of the Noise handshake, even without decrypting their contents.

##### The Stratum V2 dance

The Noise handshake is followed by a burst of messages. This matters because a miner has no reason to open a connection, complete the Noise handshake, and then wait before opening mining channels. It wants to start hashing as soon as possible.

The Noise handshake is followed by the Stratum V2 handshake. In the plot below, Wireshark shows nine data-carrying packets in the first second, excluding the TCP handshake and ACK-only packets. These are our Stratum V2 message candidates.

A packet does not necessarily represent one protocol message. A large message can split across several packets. In my lab, each packet happened to contain one complete message, but that was a coincidence:
...
We could apply the same heuristic to the protocol's other messages and responses. I will stop here because the pattern is clear and I got lazy.

At this point, the inferences look like this:
...
None of these means that "packet size X proves message Y." They become useful when we combine size, direction, timing, sequence, and protocol state.
...
Looking at the spec, we can match the 74-byte packets with the [`SetTarget`](https://github.com/stratum-mining/sv2-spec/blob/main/05-Mining-Protocol.md#5321-settarget-server---client) message. But this inference does not come from packet size alone. It also depends on an implementation detail of the SRI Pool.
...
The 60-second `SetTarget` frequency is **not a protocol requirement**. It comes from the SRI Pool's `vardiff` mechanism, so this heuristic may not work with another pool implementation.

Message direction is another useful clue. This sequence goes from the Pool to the Client, and the specification defines both `NewMiningJob` and `SetTarget` as Server -> Client messages.
...
Combining these observations increases our confidence about what is happening inside the encrypted Stratum V2 connection.

#### Inferring share rate and block events

I cannot determine the miner's hashrate because the relevant data is encrypted. After the sequence of `NewMiningJob` and `SetTarget` messages, however, the miner was probably receiving jobs that allowed it to submit shares upstream.
...
My lab is running on regtest, where finding a block is easy. This matters for the next inference.
...
I can cheat a little more here. An observer could run a Bitcoin Core node and compare the arrival times of these `SubmitShares.Success` responses with blocks found by that node on the same network.
...
On regtest, the difficulty is extremely low compared with mainnet. In this experiment, a submitted share can also satisfy the block target, which lets us correlate share submissions with blocks being found.
...
The share submission messages also reveal the **share rate**. This is not the hashrate. The payload encrypts the hashrate information.
...
This estimate becomes less useful over time because of `vardiff`. As the difficulty adjusts, connections tend to converge toward a similar share-per-minute rate. A long enough capture could still reveal a large increase or decrease in a miner's activity.

### Conclusions

* To an observer who does not know what to look for, this encrypted traffic may **not draw much attention** among other network traffic. It did not stand out to me before I understood the protocol.
* An observer who knows the protocol could identify likely Stratum V2 traffic with some confidence. Someone monitoring network traffic could use this information to identify or block suspected mining operations.
* Does this mean that Stratum V2 is broken? No. Preventing traffic fingerprinting is not one of the protocol's main goals. The protocol aims to support a more decentralized approach to template creation and mining.
* This scenario is also naive. Real traffic would be harder to analyze because of network noise and mining proxies that aggregate multiple miners behind one connection to the pool.
* Implementations can behave differently. For example, the Translator Proxy created by the SRI team opens a Mining Channel with the pool only when the first mining device connects to it. This breaks my earlier assumption about the initial packet burst. With the Translator Proxy, we could see the Noise handshake and the Stratum V2 `SetupConnection` handshake without seeing an immediate Open Channel message.

I did not try several things. I used one miner, one pool, one connection, and a known implementation on both sides. I did not build a classifier, test multiple implementations, add realistic network noise, or test these heuristics through different proxy configurations.

That was not the point of this experiment.

I wanted to know whether I could inspect an encrypted Stratum V2 connection in Wireshark and still determine what was happening.
